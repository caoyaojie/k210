        ...
        if (sensor->chip_id == MT9V034_ID) {
            ...
            switch (sensor->chip_id) {
                ...
                case OV2640_ID:
                    ...
                    //cyj add 2019-9-19
                    mp_printf(&mp_plat_print, "[MAIXPY]: ov2640_init exe over return = %d gs_bpp = %d  hmirror= %d,vflip = %d\n",init_ret,sensor->gs_bpp,sensor->set_hmirror,sensor->set_vflip);
                    break;
                // case OV7725_ID:
				// 	/*ov7725_init*/
                //     printk("find ov7725\r\n");
...
int sensor_init_dvp(mp_int_t freq)
{
    ...
    // cyj add 2019-9-19
    mp_printf(&mp_plat_print, "[MAIXPY]: begin to setup dvp \n");
    ...
    // cyj add 2019-9-19
    mp_printf(&mp_plat_print, "[MAIXPY]: sensor.size_set return is %d \n",init_ret);
    return init_ret;
}
int sensor_init_irq()
{
...
//cyj modiyf 2019-8-20

image_t image_old;
image_t *image_pr = &image_old;
...
{	
    ...
    do {
        if (streaming_cb  && image->pixels != NULL) {  //&& doublebuf
            ...
            streaming = streaming_cb(image);  //cyj modify 1019-8-23
        }
	    // Fix the BPP
        switch (sensor->pixformat) {
            case PIXFORMAT_GRAYSCALE:
	...
	g_dvp_finish_flag = 0;
        ...
        //cyj modify 2019-8-20 from dead delay to multi-task mode begin

	while (g_dvp_finish_flag == 0)  //cyj add wait to dvp finish
        {
            _ndelay(50);
            if(systick_current_millis() - start > 150)//wait for 30ms cyj modify old is 300 
                return -1;
        }
		...
		if (streaming_cb) {
			...
			// or call the streaming callback with the main FB in single buffer mode.
				...
				streaming = streaming_cb(image); //cyj modify 2019-8-23 
		}
    //*/streaming = false; //cyj modify 2019-8-22
